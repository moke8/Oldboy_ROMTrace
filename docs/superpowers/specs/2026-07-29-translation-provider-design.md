...
翻译日志保留开始、完成和失败状态。Provider 请求失败时通过可选日志回调输出一行用户可读原因，格式为 `[翻译] <模型或 Provider>: <原因>`。例如模型不存在或无权访问时输出 `[翻译] MiniMax-M2.7: 该模型不存在，或当前 Key 无权访问。`。HTTP 错误优先解析接口返回的错误代码和消息，并将常见错误转换为中文；网络、超时、响应解析和配置缺失分别给出明确原因。日志不得输出 API Key、Authorization 请求头或完整响应正文。

具体 Provider 函数和统一调度函数继续以原文作为失败回退值，不抛出异常中断批量刮削。刮削层在收到未变化结果时只记录未生成新译文，具体失败原因由紧邻的 Provider 日志说明。
